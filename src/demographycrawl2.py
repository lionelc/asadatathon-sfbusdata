...
def main():
    lldict = dict()
    #read lat lng data
    f = open("/cygdrive/e/WebDownloads/mode-report-run-5a64e76c69b4.csv" , "r")
    line = f.readline()
    line = f.readline()
    ...
    #call the web service again to get demographics info
    print len(lldict)
    ind = 1
    for elem in lldict:
    ...
    #append the columns
    colnum = 10
    f1 = open("/cygdrive/e/WebDownloads/mode-report-run-5a64e76c69b4.csv" , "r")
    f2 = open("latlng-join-demographics.csv", "w")
    line = f1.readline()
